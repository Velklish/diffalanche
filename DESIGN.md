...
- **Danger:** the primary filled `crit` instead of `acc`. There are exactly two
  in the product, the only presses that destroy review data — `Убрать и удалить`
  in the scope editor's confirmation and `Удалить` in the question a task's row
  asks before it is deleted — and it is the colour of a filled critical badge, so
