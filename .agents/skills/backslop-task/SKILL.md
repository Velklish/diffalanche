...
The subject is one task from taking it to archiving it. Tracker rules are in `docs/backlog/README.md`; this file lays out the order by role and step. `npx github:Velklish/backslop#v0.4.0` is the command recorded in `backslop.json`; below it is simply called `backslop`.
