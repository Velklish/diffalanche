...
describe("watcher", () => {
  it("rescans the edited repository alone and has the new hunk in diff.json in time", async () => {
    const file = join(root, REPO, "watched.ts");
    const elapsed: number[] = [];
    let first = 0;
    // Three edits and the median of them, the way the performance gate reads
    // its own numbers: one slow run on a busy machine is not a regression.
    for (let run = 0; run < RUNS; run += 1) {
      const mark = performance.now();
      if (run === 0) first = mark;
      await writeFile(file, `export const watched = ${run};\n`);
      ...
      expect(event.files).toContain("watched.ts");
    }

    const cache = await readDiffCache(config.dataDir, SESSION);
    const repository = cache?.repositories.find((one) => one.path === REPO);
    const added = repository?.files.find((one) => one.path === "watched.ts");
    expect(added?.hunks[0]?.lines.map((line) => line.content)).toEqual([
      `export const watched = ${RUNS - 1};`,
    ]);
    ...
    expect(median(elapsed)).toBeLessThan(BUDGET_MS + (await baseline()));
  });
  ...
  it("says nothing when the file comes back with the same bytes", async () => {
    ...
    await new Promise((done) => setTimeout(done, 6 * BUDGET_MS));
    expect(since(mark, "diff-changed")).toEqual([]);
  });
  ...
  it("reads the whole change set again when diff.json is gone", async () => {
    ...
    // review of one repository.
    const cache = await readDiffCache(config.dataDir, SESSION);
    expect(cache?.repositories).toHaveLength(PROFILES.small.repos);
  });
