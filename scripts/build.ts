...
function generateEntry(target: Target): string {
  ...
  writeFileSync(
    ...
    `// Generated by scripts/build.ts. Not checked in.\n` +
      ...
      `import { CHILD, serveChild } from "../src/core/ml/embed/child.ts";\n` +
      ...
      `} });\n` +
      `if (process.env[CHILD] === "1" && process.argv.length === 2) serveChild();\n` +
      `else {\n` +
...
await bundleNpm();
console.log(
  `dist/cli.js  ${size("dist/cli.js")}, dist/embed-child.js  ${size("dist/embed-child.js")}`,
);
copyGrammars();
...
for (const target of targets) {
  ...
  const result = await bundler.build({
    entrypoints: [generateEntry(target)],
