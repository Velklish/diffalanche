#!/usr/bin/env bun
/** Both delivery channels: the npm bundle (scripts/bundle.ts), and a binary per platform with the
 * UI, the model and the runtime's native files inside. Bun only, so `Bun.*` is allowed here. */
...
import { bundleNpm, bundler, channel } from "./bundle.ts";
...
function generateEmbeddedUi(): void {
  const root = "dist/ui";
  ...
  walk(root);
  writeFileSync(
    join(GENERATED, "ui-assets.generated.ts"),
  ...
  // Beside the entry, so the two land side by side in the binary as the npm bundle has them.
  writeFileSync(
    join(GENERATED, "embed-worker.ts"),
    `// Generated by scripts/build.ts. Not checked in.\nimport "../src/core/ml/embed/worker.ts";\n`,
  );
}

/** The entry of one target: the UI, and the model and the runtime's native files the binary
 * carries, by their release names (09-ml.md, "Delivery"); darwin-x64 carries none. */
  ...
  writeFileSync(
    ...
    `// Generated by scripts/build.ts. Not checked in.\n` +
      ...
      names
        .map(
          (name, i) =>
            `import file${i} from ${JSON.stringify(resolve(files[name] as string))} with { type: "file" };\n`,
        )
        .join("") +
      `\ndeliverFrom({ from: "binary", files: {\n` +
      names.map((name, i) => `  ${JSON.stringify(name)}: file${i},\n`).join("") +
      `} });\n` +
...
await bundleNpm();
console.log(
  `dist/cli.js  ${size("dist/cli.js")}, dist/embed-worker.js  ${size("dist/embed-worker.js")}`,
);
copyGrammars();
...
for (const target of targets) {
  ...
  const result = await bundler.build({
    entrypoints: [generateEntry(target), join(GENERATED, "embed-worker.ts")],
    compile: { target: `bun-${target.platform}-${target.arch}`, outfile },
    plugins: [channel('new URL("./embed-worker.js", import.meta.url)')],
  });
  if (!result.success) throw new AggregateError(result.logs, `build: ${outfile} did not build`);
