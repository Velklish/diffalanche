...
| browse mode | `browse` | One file of a repository of the review read whole — numbered from 1, read-only, at the working tree or the base — in place of the review, which stays folded underneath. Entered from `all files`, `Browse repo`, `B`, or global search; comments are allowed. | HANDOFF.md §8, reference/08-ui.md |
