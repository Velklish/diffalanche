...
The canonical project documentation. For current work, use `npx github:Velklish/backslop#v0.3.1 status`; for project direction, see [ROADMAP.md](ROADMAP.md); for why the system is arranged this way, see the ADRs in the table below.
...
| [backlog/](backlog/README.md) | Task tracker: one file per task, status is the directory, summary is `npx github:Velklish/backslop#v0.3.1 status` | Living |
...
Create a new ADR with `npx github:Velklish/backslop#v0.3.1 adr <slug>` **and add a row to the table above**: without the row, `npx github:Velklish/backslop#v0.3.1 lint` fails.
