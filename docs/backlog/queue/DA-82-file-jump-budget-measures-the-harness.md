...
- Once the hook calls `revealCard`, remove `data-file-index` from `FileCard` if nothing else needs it, so the next person does not take it for a production hook. It is otherwise a candidate for the sweep in [DA-59-dead-code-sweep.md](DA-59-dead-code-sweep.md).
...
- `afterPaint`'s definition of a painted frame, which the same argument touches for other rows and is [DA-55.4-settle-measures-before-paint.md](DA-55.4-settle-measures-before-paint.md).
