...
The remaining cover is thin and known to be unsound. [e2e/sidebar.spec.ts](../../../e2e/sidebar.spec.ts):89-102 does click a real `.file-row` and assert `jump < 50`, but on the small e2e fixture, from a single sample, with none of the gate's median-of-three or runner allowance — which is item 6 of [DA-60-flaky-test-hardening.md](../../archive/DA-60-flaky-test-hardening/task.md), filed there as a budget held to a stricter rule in a suite that is not the gate. So the shipped jump is asserted only there, and never on the 300-file synthetic review the budget was written for.
...
- The e2e assertion's single sample and missing allowance, which is item 6 of [DA-60-flaky-test-hardening.md](../../archive/DA-60-flaky-test-hardening/task.md).
